...
/**
 * Activity for input the checking parameters.
 */
public class MainActivity extends Activity implements OnClickListener
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        // show the alert box to the main avtivity before input the parameters into the input box.
        new AlertDialog.Builder(this).setTitle("Disclaimer").setMessage(getResources().getString(R.string.disclaimer)).setPositiveButton("Accept", null).setNegativeButton("Reject", new DialogInterface.OnClickListener()
        {
            @Override
    ...
    public void onClick(View view)
    {
        ...
        switch (view.getId())
        {
            //on click action for the check button.
            case R.id.submitButton:
                //validating all the input box,if one of the input box is null,than show the alert message.
                if (TextUtils.isEmpty(etAgeInYears.getText()) || TextUtils.isEmpty(etHeightInCm.getText()) || TextUtils.isEmpty(etWeightInKg.getText()) || TextUtils.isEmpty(etHbA1cNgsp.getText()) || TextUtils.isEmpty((CharSequence) spBinaryGender.getSelectedItem()) || TextUtils.isEmpty((CharSequence) spTreatment.getSelectedItem()))
                {
                    new AlertDialog.Builder(this).setTitle("Incomplete").setMessage(getResources().getString(R.string.incomplete)).setPositiveButton("Done", null).show();
                    break;
                }
                //get all the parameters from each input box.
                int ageInYears = TextUtils.isEmpty(etAgeInYears.getText()) ? 0 : Integer.parseInt(etAgeInYears.getText().toString());
                int heightInCm = TextUtils.isEmpty(etHeightInCm.getText()) ? 0 : Integer.parseInt(etHeightInCm.getText().toString());
                float weightInKg = TextUtils.isEmpty(etWeightInKg.getText()) ? 0 : Float.parseFloat(etWeightInKg.getText().toString());
                float hbA1cNgsp = TextUtils.isEmpty(etHbA1cNgsp.getText()) ? 0 : Float.parseFloat(etHbA1cNgsp.getText().toString());
                String binaryGender = TextUtils.isEmpty((CharSequence) spBinaryGender.getSelectedItem()) ? "" : spBinaryGender.getSelectedItem().toString();
                String treatment = TextUtils.isEmpty((CharSequence) spTreatment.getSelectedItem()) ? "" : spTreatment.getSelectedItem().toString();
                ...
                //get the checking result by a https request.
                try
                {
                    new Thread(new Runnable()
                    {
                        ...
                        public void run()
                        {
                            try
                            {
                                ...
                                //If the response is not null. Jumping to the chart activity.
            ...
            case R.id.text_view_2:
                new AlertDialog.Builder(this).setTitle("Attention").setMessage("text view 2").setPositiveButton("Yes", null).show();
                break;
            case R.id.text_view_3:
                new AlertDialog.Builder(this).setTitle("Attention").setMessage("text view 3").setPositiveButton("Yes", null).show();
                break;
            case R.id.text_view_4:
                new AlertDialog.Builder(this).setTitle("Attention").setMessage("text view 4").setPositiveButton("Yes", null).show();
                break;
            case R.id.about_us_show:
                new AlertDialog.Builder(this).setTitle("About Us").setMessage(getResources().getString(R.string.about_us_desc)).setPositiveButton("Done", null).show();
                break;
            case R.id.hbaic_desc_show:
                new AlertDialog.Builder(this).setTitle("HBA1c").setMessage(getResources().getString(R.string.hba1c_desc)).setPositiveButton("Done", null).show();
                break;
            case R.id.bmi_desc_show:
                new AlertDialog.Builder(this).setTitle("BMI").setMessage(getResources().getString(R.string.bmi_desc)).setPositiveButton("Done", null).show();
                break;
            default:
                break;
        }
