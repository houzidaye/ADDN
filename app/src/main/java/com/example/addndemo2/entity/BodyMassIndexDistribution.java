...
public class BodyMassIndexDistribution implements Serializable {
	private float min;
	private float max;
	private float mean;
	...
	private float bmiPercentileValue = 0;
	public static float multiple = 100;

	public BodyMassIndexDistribution() {
	}

	public BodyMassIndexDistribution(float min, float max, float mean,
			float standardDeviation, float twentyFifthPercentileValue,
			float fiftiethPercentileValue, float seventyFifthPercentileValue) {
		this.min = min;
		this.max = max;
		this.mean = mean;
...
//		this.multiple = Constants.chartAxisY * 10 / maxValue;
	...
	public BodyMassIndexDistribution initByJson(String json)
			throws JSONException {
		JSONObject jObj = new JSONObject(json);
		JSONObject bodyMassJson = jObj
				.getJSONObject("bodyMassIndexDistribution");
		...
		float twentyFifthPercentileValue = (float)bodyMassJson
				.getDouble("twentyFifthPercentileValue");
		float fiftiethPercentileValue = (float)bodyMassJson
				.getDouble("fiftiethPercentileValue");
		float seventyFifthPercentileValue = (float)bodyMassJson
				.getDouble("seventyFifthPercentileValue");
		return new BodyMassIndexDistribution(min, max, mean, standardDeviation,
				twentyFifthPercentileValue, fiftiethPercentileValue,
				seventyFifthPercentileValue);
	}

	public float getBmiPercentileValue() {
		return bmiPercentileValue;
	}

	public void setBmiPercentileValue(float bmiPercentileValue) {
		this.bmiPercentileValue = bmiPercentileValue;
	}
	...
	public String toString() {
		return " min:" + min + " max:" + max + " mean:" + mean
				...
				+ " twentyFifthPercentileValue:" + twentyFifthPercentileValue
				+ " fiftiethPercentileValue:" + fiftiethPercentileValue
				+ " seventyFifthPercentileValue:" + seventyFifthPercentileValue;
	}
	...
	public List<com.example.addndemo2.chartEntity.Point> getPoint()
	{
		...
		pointList.add(new Point(Constants.endAxisX - 300, (maxAxisY - bmiPercentileValue) * multiple, "Your BMI " + String.valueOf(bmiPercentileValue), Color.parseColor("#FFFFFF")));
	...
	public List<com.example.addndemo2.chartEntity.Line> getLine()
	{
		...
		float yourLineAxisY = (maxAxisY - bmiPercentileValue) * multiple;
	...
	public List<Rectangle> getBodyMassIndexDistributionRectangle()
	{
	...
	public List<Line> getBodyMassIndexDistributionLines(){
		...
		Line minLine = LineChartUtil.createLine(min, initAxisXValue,axisXSize, Color.parseColor("#1EAD64"),true);
		Line minOnePointLine = LineChartUtil.createOnePointLine(axisXmid, min, Color.parseColor("#1EAD64"), "Min");
		Line maxLine = LineChartUtil.createLine(max, initAxisXValue,axisXSize, Color.RED,true);
		Line maxOnePointLine = LineChartUtil.createOnePointLine(axisXmid, max, Color.RED, "Max");
		Line yourLine = LineChartUtil.createLine(bmiPercentileValue, 0,axisXMaxSize, Color.parseColor("#00FFFF"),false);
		Line yourOnePointLine = LineChartUtil.createOnePointLine(axisXmid, bmiPercentileValue, Color.parseColor("#00FFFF"), "Your BMI");
		...
		Line twentyFifthPercentileValueLine = LineChartUtil.createLine(twentyFifthPercentileValue, initAxisXValue,axisXSize, Color.parseColor("#F9AB21"),true);
		Line twentyFifthPercentileValueOnePointLine = LineChartUtil.createOnePointLine(axisXSize+initAxisXValue, twentyFifthPercentileValue, Color.parseColor("#F9AB21"), "");
		Line fiftiethPercentileValueLine = LineChartUtil.createLine(fiftiethPercentileValue, initAxisXValue,axisXSize, Color.parseColor("#E37E2B"),true);
		Line fiftiethPercentileValueOnePointLine = LineChartUtil.createOnePointLine(axisXSize+initAxisXValue, fiftiethPercentileValue, Color.parseColor("#E37E2B"), "");
...
//		inputList.add(new ChartLine(standardDeviation,standardDeviationLine));
		...
		for (ChartLine line : inputList) {
			...
			Log.d("bmi sort line", String.valueOf(line.getPointValue()));
		}
